    ...
    var Exam_Time = Math.floor(0);

    fetch("questions.json").then(res => {
        console.log(res)
        return res.json()
    }).then(loadedQuestions => {
        Exam_Time = Math.floor(30);
    ...
    function start(questions) {
        ...
        let question_no_sidebar = document.getElementsByClassName('question_no_sidebar');
        ...
        function showQuestion(quest) {
            ...
            chng.style.backgroundColor = 'blue';
        ...
        next.addEventListener('click', function run() {
            if (currIndex < l - 1) {
                ...
                localStorage.setItem('curr', JSON.stringify(currIndex));
                showQuestion(questions[currIndex])
                reset_sidebar_button_color();

                question_no_sidebar[currIndex].style.color = 'white';
                question_no_sidebar[currIndex].style.backgroundColor = 'blue';
            }
        });
        ...
        function change_color(coount) {
            reset_color()
            options_class[coount].style.backgroundColor = 'blue'
